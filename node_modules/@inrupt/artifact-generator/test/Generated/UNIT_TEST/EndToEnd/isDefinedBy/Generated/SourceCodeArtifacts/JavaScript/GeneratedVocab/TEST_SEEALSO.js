/**
 * Generated by the Inrupt Artifact Generator [@inrupt/artifact-generator], version [3.1.1]
 * as part of artifact: [generated-vocab-common-TEST], version: [10.11.12]
 * on 'Monday, October 2, 2023 11:19 AM'.
 *
 * Vocabulary built from vocab list file: [./test/resources/yamlConfig/vocab-strict.yml].
 * The generator detected the following terms in the source vocabulary:
 *  - Classes: [1]
 *  - Properties: [0]
 *  - Literals: [0]
 *  - Constant IRIs: [0]
 *  - Constant strings: [0]
 *
 * [Generator provided] - Test generated VocabTerm handling of rdfs:seeAlso.
 */

// We prefix our local variables with underscores to (hopefully!) prevent
// potential names clashes with terms from vocabularies.
const { VocabTerm: _VocabTerm, getLocalStore } = require("@inrupt/solid-common-vocab");
const _DataFactory = require("@rdfjs/data-model");

const { namedNode: _namedNode } = _DataFactory;

function _NS(localName) {
  return _namedNode(`https://ex.com/vocab/test/${localName}`);
}

/**
 * [Generator provided] - Test generated VocabTerm handling of rdfs:seeAlso.
 *
 * Namespace IRI: [https://ex.com/vocab/test/]
 */
const TEST_SEEALSO = {
  PREFIX: "test_seeAlso",
  NAMESPACE: "https://ex.com/vocab/test/",
  PREFIX_AND_NAMESPACE: { "test_seeAlso": "https://ex.com/vocab/test/" },
  NS: _NS,

  // *****************
  // All the Classes.
  // *****************

  /**
   * Some test class
   *
   * This term has a label (in language [NoLocale]), but no long-form descriptions at all (i.e., the vocabulary doesn't provide any 'rdfs:comment' or 'dcterms:description' metadata).
   *
   * See also:
   *  - https://ex.com/vocab/test/
   *  - http://www.w3.org/1999/02/22-rdf-syntax-ns#
   *  - http://www.w3.org/2000/01/rdf-schema#
   */
  SomeClass: new _VocabTerm(
    _NS("SomeClass"),
    _DataFactory,
    getLocalStore(),
    true
  )
    .addType(_namedNode("http://www.w3.org/2000/01/rdf-schema#Class"))
    .addSeeAlso(_namedNode("https://ex.com/vocab/test/"))
    .addSeeAlso(_namedNode("http://www.w3.org/1999/02/22-rdf-syntax-ns#"))
    .addSeeAlso(_namedNode("http://www.w3.org/2000/01/rdf-schema#"))
    .addLabelNoLanguage(`Some test class`),

}

module.exports = TEST_SEEALSO;
