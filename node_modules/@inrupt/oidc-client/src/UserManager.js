...
export class UserManager extends OidcClient {
  ...
  signinPopup(args = {}) {
    ...
    return this._signin(args, this._popupNavigator, {
      startUrl: url,
      popupWindowFeatures:
        args.popupWindowFeatures || this.settings.popupWindowFeatures,
      popupWindowTarget:
        args.popupWindowTarget || this.settings.popupWindowTarget,
    }).then((user) => {
      if (user) {
        if (user.profile && user.profile.sub) {
          Log.info(
  ...
  signinPopupCallback(url) {
    return this._signinCallback(url, this._popupNavigator)
      ...
      .catch((err) => {
        Log.error(
          "UserManager.signinPopupCallback error: " + err && err.message
        );
      });
  }
  ...
  _signinSilentIframe(args = {}) {
    ...
    return this._signin(args, this._iframeNavigator, {
      ...
      silentRequestTimeout:
        args.silentRequestTimeout || this.settings.silentRequestTimeout,
    }).then((user) => {
      if (user) {
        if (user.profile && user.profile.sub) {
          Log.info(
  ...
  _signin(args, navigator, navigatorParams = {}) {
    return this._signinStart(args, navigator, navigatorParams).then(
      (navResponse) => {
  ...
  signoutRedirect(args = {}) {
    ...
    let postLogoutRedirectUri =
      args.post_logout_redirect_uri || this.settings.post_logout_redirect_uri;
  ...
  signoutRedirectCallback(url) {
    return this._signoutEnd(url || this._redirectNavigator.url).then(
      (response) => {
  ...
  _signout(args, navigator, navigatorParams = {}) {
    return this._signoutStart(args, navigator, navigatorParams).then(
      (navResponse) => {
    ...
    return navigator.prepare(navigatorParams).then((handle) => {
      ...
      return this._loadUser()
        .then((user) => {
          ...
          var revokePromise = this._settings.revokeAccessTokenOnSignout
  ...
  _revokeInternal(user, required) {
    if (user) {
      ...
      return this._revokeAccessTokenInternal(access_token, required).then(
        (atSuccess) => {
          return this._revokeRefreshTokenInternal(refresh_token, required).then(
            (rtSuccess) => {
  ...
  _revokeRefreshTokenInternal(refresh_token, required) {
    ...
    return this._tokenRevocationClient
      .revoke(refresh_token, required, "refresh_token")
      .then(() => true);
  }
