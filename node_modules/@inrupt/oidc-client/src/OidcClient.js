...
export class OidcClient {
  ...
  createSigninRequest(
    ...
    return this._metadataService.getAuthorizationEndpoint().then((url) => {
      ...
      return stateStore
        .set(signinState.id, signinState.toStorageString())
        .then(() => {
  ...
  readSigninResponseState(url, stateStore, removeState = false) {
    ...
    var stateApi = removeState
      ? stateStore.remove.bind(stateStore)
      : stateStore.get.bind(stateStore);
  ...
  createSignoutRequest(
    ...
    post_logout_redirect_uri =
      post_logout_redirect_uri || this._settings.post_logout_redirect_uri;
    extraQueryParams = extraQueryParams || this._settings.extraQueryParams;
